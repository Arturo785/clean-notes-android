...
// Every action that can be fired up by the user and triggers the use cases

sealed class NoteListStateEvent :
    StateEvent { // uses the abstraction from the business state interface

    class InsertNewNoteEvent(
        val title: String
    ) : NoteListStateEvent() {

        // the ones from the interface
